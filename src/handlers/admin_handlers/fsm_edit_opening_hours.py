...
@router.callback_query(StoreAdminCbData.filter(F.type_press == 'edit-hours'))
...
async def process_wait_opening_time(
    ...
    if not time_pattern.match(text):
        await message.reply(
            "Текст должен быть в формате ЧЧ:ММ "
            "(например: 14:00). Пожалуйста, введите корректное время."
        )
        return
    else:
        await state.update_data(opening_time=message.text)
        await message.answer(
            text='Введите время окончания работы в формате "чч:мм" '
                 '(например: 23:00)',
        )
...
async def process_wait_closing_time(
    ...
    if not time_pattern.match(text):
        await message.reply(
            "Текст должен быть в формате ЧЧ:ММ (например: 14:00). "
